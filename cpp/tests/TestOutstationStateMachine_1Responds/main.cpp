...
qDebug()<<"********SUITE('TestOutstationStateMachine_1Responds')********";
...
DatabaseConfig tmp = analog_input_in_DatabaseHelpers(1);
    ...
    std::string name("C0 01 1E 00 06");       
    SendToOutstation_in_OutstationTestObject(&t, name);  

    std::string temp = PopWriteAsHex_in_MockLowerLayer(&(t.lower));

qDebug()<<"REQUIRE(t.lower->PopWriteAsHex() == 'C0 81 80 00 1E 01 00 00 00 02 00 00 00 00')";
std::cout << "temp= " << temp<<'\n';
...
////    t.Transaction([](IUpdateHandler& db) { db.Update(Analog(1, Flags(0x01)), 0); });
    Transaction_in_OutstationTestObject(&t, update_in_1Responds);//void (*apply)(IUpdateHandler*));

    // repeat the read request
////    t.SendToOutstation("C0 01 1E 00 06");
    std::string name2("C0 01 1E 00 06");       
