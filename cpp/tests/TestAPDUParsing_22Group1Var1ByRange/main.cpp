...
{
qDebug()<<"";
qDebug()<<"Indexed<Binary> value(Binary(false), 4)";
...
{
qDebug()<<"";
qDebug()<<"Indexed<Binary> value(Binary(false), 5)";
...
{
qDebug()<<"";
qDebug()<<"Indexed<Binary> value(Binary(true), 6)";
qDebug()<<"REQUIRE((value == mock.staticBinaries[3]))";
Indexed_for_Binary value = mock->staticBinaries[3];
qDebug()<<"value.index= "<<value.index;
boolean src = value.value.tTypedMeasurement_for_Boolean.value;
qDebug()<<"boolean src= "<<src;
}
...
qDebug()<<"********SUITE('22Group1Var1ByRange')********";
void validate(MockApduHeaderHandler*);
{
  std::string name("01 01 00 03 06 09");       
ParseResult_uint8_t temp = TestComplex(name, validate);
qDebug()<<"TestComplex('01 01 00 03 06 09', ParseResult::OK, 1, [](MockApduHeaderHandler& mock)";
