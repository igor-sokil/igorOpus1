...
DatabaseConfig database = binary_input_in_DatabaseHelpers(10);

    for (auto& item : database.binary_input)
    {
        item.second.eEventConfig.svariation = StaticBinaryVariation_Group1Var1;
    }
...
// Установить все значения в ONLINE, чтобы они не получили повышение
////    t.Transaction([](IUpdateHandler& handler) { handler.Modify(FlagsType::BinaryInput, 0, 9, 0x01); });
    Transaction_in_OutstationTestObject(&t, apply_in_41ReadGrp1Var1);//void (*apply)(IUpdateHandler*));
...
qDebug()<<"REQUIRE(t.lower->PopWriteAsHex() == 'C0 81 80 00 01 01 00 00 09 00 00')";
std::cout << "temp= " << temp<<'\n';
