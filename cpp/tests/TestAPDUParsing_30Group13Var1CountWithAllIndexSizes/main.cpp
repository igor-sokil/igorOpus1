...
{
qDebug()<<"REQUIRE(1 == mock.binaryCommandEvents.size())";
 uint16_t size = mock->binaryCommandEvents.size();
...
qDebug()<<"********SUITE('30Group13Var1CountWithAllIndexSizes')********";
void validate(MockApduHeaderHandler*);
{
  std::string name("0D 01 17 01 09 81");       
ParseResult_uint8_t temp = TestComplex(name, validate);
qDebug()<<"TestComplex('0D 01 17 01 09 81', ParseResult::OK, 1, [](MockApduHeaderHandler& mock)";
qDebug()<<"ParseResult_uint8_t result= "<<temp;
}

{
  std::string name("0D 01 28 01 00 09 00 81");       
ParseResult_uint8_t temp = TestComplex(name, validate);
qDebug()<<"TestComplex('0D 01 28 01 00 09 00 81', ParseResult::OK, 1, [](MockApduHeaderHandler& mock)";
qDebug()<<"ParseResult_uint8_t result= "<<temp;
}
/*
