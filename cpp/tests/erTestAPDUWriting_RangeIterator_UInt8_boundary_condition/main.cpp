...
//    APDURequest request(APDUHelpers::Request(FunctionCode::READ, 6));
     APDURequest request = 
                Request_in_APDUHelpers_static(FunctionCode_READ, 6);
...
//    REQUIRE(writer.WriteHeader(Group60Var1::ID(), QualifierCode::ALL_OBJECTS));
   boolean temp = WriteHeader_in_HeaderWriter(&writer, ID_in_Group60Var1_static(), QualifierCode_ALL_OBJECTS);
     qDebug()<<"REQUIRE(writer.WriteHeader(Group60Var1::ID(), QualifierCode::ALL_OBJECTS))";
     qDebug()<<"boolean temp= "<<temp;
//    REQUIRE(!writer.WriteHeader(Group60Var1::ID(), QualifierCode::ALL_OBJECTS));
   temp = WriteHeader_in_HeaderWriter(&writer, ID_in_Group60Var1_static(), QualifierCode_ALL_OBJECTS);
     qDebug()<<"REQUIRE(!writer.WriteHeader(Group60Var1::ID(), QualifierCode::ALL_OBJECTS))";
     qDebug()<<"boolean temp= "<<temp;
...
//    REQUIRE("C0 01 3C 01 06" == HexConversions::to_hex(request.ToRSeq()));
     ...
     qDebug()<<"REQUIRE('C0 01 3C 01 06' == HexConversions::to_hex(request.ToRSeq()))";
