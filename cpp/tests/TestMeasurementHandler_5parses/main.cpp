...
qDebug()<<"REQUIRE(soe.TotalReceived() == 1)";
qDebug()<<"tmp= "<<TotalReceived_in_MockSOEHandler(soe);
qDebug()<<"REQUIRE(soe.binarySOE.size() == 1)";
qDebug()<<"tmp= "<<soe->binarySOE.size();

////        const auto record = soe.binarySOE.find(0XAA);
        const auto record = soe->binarySOE.find(8);
//        Record_for_Binary_in_MockSOEHandler record = soe->binarySOE.find(0XAA);
////        REQUIRE(record->second.meas.time.quality == TimestampQuality::SYNCHRONIZED);
qDebug()<<"REQUIRE(record->second.meas.time.quality == TimestampQuality::SYNCHRONIZED)";
qDebug()<<"tmp= "<<record->second.meas.tTypedMeasurement_for_Boolean.mMeasurement.timeDNPTime.quality;
qDebug()<<"TimestampQuality::SYNCHRONIZED= "<<TimestampQuality_SYNCHRONIZED;

////        REQUIRE(record->second.meas.time.value == 8);
qDebug()<<"REQUIRE(record->second.meas.time.value == 8)";
qDebug()<<"tmp= "<<record->second.meas.tTypedMeasurement_for_Boolean.mMeasurement.timeDNPTime.value;
...
qDebug()<<"********SUITE('3parses g2v1 w/ invalid timestamp quality')********";
...
////    auto objects = "33 01 07 01 07 00 00 00 00 00 02 03 17 01 08 81 01 00";
 std::string objects = "33 01 07 01 07 00 00 00 00 00 02 03 17 01 08 81 01 00";
