...
qDebug()<<"********SUITE('2calls write multiple times for different variations')********";
...
////        storage.Update(Event<BinarySpec>(Binary(true), 0, EventClass::EC1, EventBinaryVariation::Group2Var2)));
  ...
  Event_for_BinarySpec_in_Event_for_BinarySpecOver2(&eEvent_for_BinarySpec2,
      &bBinary2, 0,
      EventClass_EC1,
      EventBinaryVariation_Group2Var2);

//boolean Update_for_BinarySpec_in_EventStorage(EventStorage *pEventStorage, Event_for_BinarySpec* evt);
  tempBool = Update_BinarySpec_in_EventStorage(&storage, &eEvent_for_BinarySpec2);
  qDebug()<<"REQUIRE_FALSE(storage.Update(Event<BinarySpec>(Binary(true), 0, EventClass::EC1, EventBinaryVariation::Group2Var2))))";
  qDebug()<<"tempBool= "<<tempBool;
...
////    handler.Expect(EventBinaryVariation::Group2Var2, 1);
    Expect_for_Binary_in_MockEventWriteHandler(&handler, EventBinaryVariation_Group2Var2, 1);
