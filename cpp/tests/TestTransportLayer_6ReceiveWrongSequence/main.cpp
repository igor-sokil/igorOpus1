...
qDebug()<<"********SUITE('6ReceiveWrongSequence')********";
...
////    test.link.SendUp(test.GetData("82")); // _/FIN/2
   std::string name2 = "82";
   std::string stmp2 = GetData_in_TransportTestObject(&test, name2, 0, MAX_TPDU_PAYLOAD);
...
Transport_StackStatistics ttmp = GetStatistics_in_TransportLayer(&(test.transport));
  ...
  std::cout<<"stmp1= "<<stmp1<<std::endl;
  std::cout<<"stmp2= "<<stmp2<<std::endl;
  qDebug()<<"REQUIRE(test.transport.GetStatistics().rx.numTransportIgnore == 1)";
  std::cout<<"numTransportIgnore= "<<ttmp.rx.numTransportIgnore<<std::endl;
