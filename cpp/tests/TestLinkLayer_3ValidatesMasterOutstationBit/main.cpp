...
qDebug()<<"********SUITE('3ValidatesMasterOutstationBit')********";
    LinkLayerTest t;
...
qDebug()<<"REQUIRE(t.link.GetStatistics().numBadMasterBit == 1)";
qDebug()<<"t.link.GetStatistics().numBadMasterBit ="<<lss->numBadMasterBit;

/*
// Check that once the layer comes up, validation errors can occur
// Проверяем, что после появления слоя могут возникнуть ошибки проверки
