...
qDebug()<<"********SUITE('3Formats one-byte qualifier properly if enough space is available')********";
...
boolean btemp = Write_in_TypedCommandHeader_for_AnalogOutputInt16(&header,
              &writer, IndexQualifierMode_allow_one_byte);
std::cout<<"REQUIRE(header.Write(writer, IndexQualifierMode::allow_one_byte))"<<'\n';
...
////    REQUIRE(hex == "29 02 17 02 0A 07 00 00 0B 08 00 00");
  ...
  qDebug()<<"REQUIRE(hex == '29 02 17 02 0A 07 00 00 0B 08 00 00')";

/*
TEST_CASE(SUITE("3Formats one-byte qualifier properly if enough space is available"))
ѕравильно форматирует однобайтовый квалификатор, если доступно достаточно места.
{
    TypedCommandHeader<AnalogOutputInt16> header(Group41Var2::Inst());
    header.Add(AnalogOutputInt16(7), 10);
    header.Add(AnalogOutputInt16(8), 11);
