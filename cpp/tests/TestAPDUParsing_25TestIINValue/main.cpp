...
{
qDebug()<<"REQUIRE(1 == mock.iinBits.size())";
 uint16_t size = mock->iinBits.size();
qDebug()<<"uint16_t size ="<<size;
  if(size!=1) return;
{
qDebug()<<"";
qDebug()<<"REQUIRE_FALSE(mock.iinBits[0].value.value)";
Indexed_for_IINValue value = mock->iinBits[0];
qDebug()<<"value.value.value="<<value.value.value;

/*
qDebug()<<"ControlRelayOutputBlock crob(OperationType::LATCH_ON, TripCloseCode::NUL, false, 1, 100, 200)";
...
qDebug()<<"********SUITE('25TestIINValue')********";
void validate(MockApduHeaderHandler*);
{
  std::string name("50 01 00 07 07 00");       
ParseResult_uint8_t temp = TestComplex(name, validate);
qDebug()<<"TestComplex('50 01 00 07 07 00', ParseResult::OK, 1, [](MockApduHeaderHandler& mock)";
