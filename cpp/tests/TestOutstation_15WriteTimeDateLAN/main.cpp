...
qDebug()<<"********SUITE('15WriteTimeDateLAN')********";
...
{
    std::string name("C2 02 32 03 07 01 D2 04 00 00 00 00");       
    SendToOutstation_in_OutstationTestObject(&t, name);  

    std::string temp = PopWriteAsHex_in_MockLowerLayer(&(t.lower));

qDebug()<<"REQUIRE(t.lower->PopWriteAsHex() == 'C2 81 80 04')";
std::cout << "temp= " << temp<<'\n';
}
/*
TEST_CASE(SUITE("15WriteTimeDateLAN - Rejects write with bad sequence number"))
//ќтклон€ет запись с неверным пор€дковым номером
{
    OutstationConfig config;
