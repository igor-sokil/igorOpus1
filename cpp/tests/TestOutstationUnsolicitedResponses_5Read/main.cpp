...
////    cfg.params.noDefferedReadDuringUnsolicitedNullResponse = true;//нет отложенного чтения во время незапрошенного нулевого ответа
...
Чтение во время нулевой отмены с обходным решением
{
    OutstationConfig cfg;
    cfg.params.allowUnsolicited = true;
    cfg.params.unsolConfirmTimeout = TimeDuration::Seconds(5);
