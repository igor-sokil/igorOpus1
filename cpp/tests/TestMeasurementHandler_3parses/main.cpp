...
////        REQUIRE(record->second.meas.time.quality == TimestampQuality::INVALID);
qDebug()<<"REQUIRE(record->second.meas.time.quality == TimestampQuality::INVALID)";
qDebug()<<"tmp= "<<record->second.meas.tTypedMeasurement_for_Boolean.mMeasurement.timeDNPTime.quality;
qDebug()<<"TimestampQuality::INVALID= "<<TimestampQuality_INVALID;
...
////    auto objects = "02 01 17 01 AA 81";
 std::string objects = "02 01 17 01 AA 81";
 TestObjectHeaders( objects, ParseResult_OK, verify);

/*
TEST_CASE(SUITE("3parses g2v1 w/ invalid timestamp quality"))
{
    auto verify = [](MockSOEHandler& soe) {
        const auto record = soe.binarySOE.find(0XAA);
        REQUIRE(record != soe.binarySOE.end());        
