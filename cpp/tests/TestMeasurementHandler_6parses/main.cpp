...
////        REQUIRE(record->second.meas.time.quality == TimestampQuality::SYNCHRONIZED);
qDebug()<<"REQUIRE(record->second.meas.time.quality == TimestampQuality::UNSYNCHRONIZED)";
qDebug()<<"tmp= "<<record->second.meas.tTypedMeasurement_for_Boolean.mMeasurement.timeDNPTime.quality;
qDebug()<<"TimestampQuality::UNSYNCHRONIZED= "<<TimestampQuality_UNSYNCHRONIZED;
...
qDebug()<<"********SUITE('6parses g2v3 w/ unsynchronized CTO')********";
...
/*
TEST_CASE(SUITE("6parses g2v3 w/ unsynchronized CTO"))
{
    auto verify = [](MockSOEHandler& soe) {
        const auto record = soe.binarySOE.find(8);
        REQUIRE(record != soe.binarySOE.end());
