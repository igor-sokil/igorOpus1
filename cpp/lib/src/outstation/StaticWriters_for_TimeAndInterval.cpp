...
////        if (!writer.Write(elem.second.value))
    if (!Write_in_RangeWriteIterator_for_UInt8_TimeAndInterval(writer, &elem.second.value))//.tTypedMeasurement_for_Double64.value))
    {
      return false;
    }
...
//    TimeAndInterval* value);
////        if (!writer.Write(elem.second.value))
    if (!Write_in_RangeWriteIterator_for_UInt16_TimeAndInterval(writer, &elem.second.value))//.tTypedMeasurement_for_Double64.value))
    {
      return false;
    }
